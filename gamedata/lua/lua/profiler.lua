...
--local module = {}
local getTime = CurrentTime --os.clock
...
local function singleSearchReturn(inputString, search)
  ...
  if string.find(inputString, search) then -- luacheck: ignore
    return true
  else
    return false
  end
...
local function rebuildColumnPatterns()
  ...
  divider = charRepetition(string.len(formatHeader) - 1, "-").."\n"
...
local function functionReport(information)
  ...
  elseif string.sub(src, string.len(src) - 3, string.len(src)) == ".lua" then
    src = string.sub(src, 1, string.len(src) - 4)
  end
  local name = information.name
  if not name then
    name = "Anon"
--  elseif string.sub(name, #name - 1, #name) == "_l" then
  elseif string.sub(name, string.len(name) - 1, string.len(name)) == "_l" then
...
local function attachPrintFunction(fn, verbose)
  printFun = fn
